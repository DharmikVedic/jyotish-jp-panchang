...
export default function VaishnavaPaushEkadashi() {
      ...
      {loader || data == "" ? (
        ...
        <div className='bg-zinc-100 min-h-screen pt-10 pb-28 px-5'>
          <div className='max-w-[750px]  mx-auto flex flex-col gap-14'>
            <FestivalDetailCard
              festival_name={"Vaishnava Paush Putrada Ekadashi"}
              date={data?.festival_date}
            />
            <Detail
              festival_date={data?.festival_date}
              harivarsra={data?.hari_vasara}
              muhurat={data?.paran_muhurta}
              tithistart={data?.tithi_start}
              tithiend={data?.tithi_end}
              name={"Vaishnava Paush Putrada Ekadashi"}
            />
          </div>
        </div>
      )}
