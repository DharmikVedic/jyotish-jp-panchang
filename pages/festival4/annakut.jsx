...
export default function Annakut() {
      ...
      {loader || data == "" ? (
        ...
        <div className='bg-zinc-100 min-h-screen pt-10 pb-28 px-5'>
          <div className='max-w-[750px]  mx-auto flex flex-col gap-20'>
            <FestivalDetailCard
              festival_name='Govardhan Puja'
              date={data?.festival_date}
            />
            <div className='relative border-2  border-sky-500 bg-white flex flex-col gap-8 pt-10 pb-5 px-5 md:p-10'>
              ...
              <h6 className='text-center md:text-lg'>
                ...
                <span className='text-red-600 font-semibold'>
                  on {convert_Date_to_redable(data?.festival_date)}
                </span>
              </h6>
              {/* punya kaal */}
              <div className='flex md:text-lg  bg-zinc-50 border-2 md:text-lg flex-col gap-2 bg-white p-5 text-center rounded'>
                <p className='text-yellow-600 font-semibold'>
                  Annakut Pratahkala Muhurat -{" "}
                  {data?.muhurta[0]?.muhurta_start_time} to{" "}
                  {data?.muhurta[0]?.muhurta_end_time}
                </p>
                <p className='text-yellow-600 font-semibold'>
                  Annakut Sayankala Muhurat -{" "}
                  {data?.muhurta[1]?.muhurta_start_time} to{" "}
                  {data?.muhurta[1]?.muhurta_end_time}
                </p>
              </div>
              <p>
                Pratipada Tithi Begins -{" "}
                <span className='text-yellow-600'>{data?.tithi_start}</span>
                <br />
                Pratipada Tithi Ends -{" "}
                <span className='text-yellow-600'>{data?.tithi_end}</span>
              </p>
            </div>
          </div>
        </div>
